...
Demo mode emits the same call lifecycle events as a call but runs a local AI-vs-AI text negotiation. Human mode uses Twilio outbound calling, TwiML `<Gather input="speech">`, and Twilio's returned `SpeechResult`. When browser takeover is configured, the signed-in owner can join an active call through the Twilio Voice JavaScript SDK; Ringside redirects the provider phone leg and browser participant into a temporary Conference, then resumes Gather mode when control returns. This is not a persistent media-stream or continuous transcription runtime. Maya synthesis is converted from raw PCM to 8 kHz WAV with ffmpeg; if synthesis is unavailable, TwiML falls back to `<Say>`.
