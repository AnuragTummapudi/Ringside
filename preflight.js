...
const checks = [
  ...
  ['Browser takeover uses owner-scoped short-lived Voice SDK tokens', () => files.server.includes("takeover/token', requireUser") && files.server.includes('createBrowserVoiceToken') && files.server.includes('ttl: 300') && files.dashboard.includes('@twilio/voice-sdk') && !files.dashboard.includes('TWILIO_API_KEY_SECRET')],
  ['Browser takeover conference returns safely to the agent', () => files.server.includes('human-conference') && files.server.includes('human-resume') && files.server.includes('resumeAgentAfterTakeover')],
